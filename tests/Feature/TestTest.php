...
class TestTest extends TestCase
{
    ...
    public function test_anyone_can_submit_a_test()
    {
        $test = factory(Test::class)->create();
        $data = ['file' => (new FileFactory)->create('results.pdf')];

        $patient = factory(Patient::class)->create();
        Passport::actingAs($patient->user);

        $response = $this->json('POST', "api/v1/tests/{$test->id}/results", $data);

        $response->assertStatus(200);

        $this->assertNotEmpty($response->original['data']['attributes']['results_key']);
    }

    public function test_a_file_is_needed_to_submit_a_test()
    {
        $test = factory(Test::class)->create();
        ...
        $response = $this->json('POST', "api/v1/tests/{$test->id}/results", []);
