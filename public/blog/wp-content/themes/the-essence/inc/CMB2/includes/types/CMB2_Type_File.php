...
class CMB2_Type_File extends CMB2_Type_File_Base {
	...
	public function render() {
		...
		$output .= sprintf( '<input class="cmb2-upload-button button" type="button" value="%s" />', esc_attr( $this->_text( 'add_upload_file_text', __( 'Add or Upload File', 'the-essence' ) ) ) );
		...
		if ( ! empty( $meta_value ) ) {
			...
			if ( $this->is_valid_img_ext( $meta_value ) ) {
				...
				if ( $_id_value ) {
					...
					$image = '<img alt="" style="max-width: ' . absint( $size ) . 'px; width: 100%; height: auto;" src="' . $meta_value . '"/>';
				}
