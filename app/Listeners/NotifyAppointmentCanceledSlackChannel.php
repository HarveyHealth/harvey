...
class NotifyAppointmentCanceledSlackChannel implements ShouldQueue
{
    ...
    public function handle(AppointmentCanceled $event)
    {
        ...
        $message = ":red_circle: *[Appointment Canceled]*: Patient: *{$patient->user->fullName()}* with {$practitioner->user->fullName()} on {$appointment_at->format('M j')} at {$appointment_at->format('g:ia')}";
