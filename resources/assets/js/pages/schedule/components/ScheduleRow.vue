...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            startBlock() {
                return this.timeBlocks.filter(block => block.start_value === this.schedule.attributes.start_time)[0];
            },
            displayStartBlock() {
                return `${this.startBlock.title} (${this.startBlock.subtitle})`;
            },
            stopBlock() {
                return this.timeBlocks.filter(block => block.stop_value === this.schedule.attributes.stop_time)[0];
            },
            displayStopBlock() {
                return `${this.stopBlock.title} (${this.stopBlock.subtitle})`;
            },
            displayDuration() {
                ...
                const minutes = parseInt(duration.asMinutes())-hours*60 > 0 ? .5 : 0;
